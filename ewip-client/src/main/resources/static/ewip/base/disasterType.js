...
layui.use(['table','form','laytpl','layer', 'selectTree', 'zTree', 'disaster'], function(){
    ...
    table.render({
        id: 'table'
        ,elem: '#table'
        ,url:'/client/disasterType/select'
        ,even: true
        ,page:true
        ,height: 'full-165'
                ...
                ,yes: function(index, layero){
                    ...
                    form.on("submit(submitAddBtn)", function(data){
                        ...
                        submitServer({
                            ...
                            ,type: 'POST'
                            ,param: data.field
                            ,url: '/client/disasterType/insert'
                        });
                    });
                    // 触发表单按钮点击事件
                    $("#submitAddBtn").click();
        ...
        ,'deleteBarBtn': function(){
            ...
            layer.confirm('确定删除这批数据？', function(index){
                ...
                submitServer({
                    ...
                    ,url: '/client/disasterType/delete'
                });
            });
        }
        /**
        ...
        ,'deleteOption': function (obj) {
            layer.confirm('确定删除该数据？', function(index){
                ...
                submitServer({
                    ...
                    ,param: null
                    ,type: 'DELETE'
                    ,url: '/client/disasterType/delete/' + obj.data.id,
                });
            });
        }
        /**
                    ...
                    laytpl(updatePop.innerHTML).render(param, function(html){
                        ...
                        form.on('select(color)',  data => {
                            ...
                            if(flag=="红色") {
                                ...
                                $("#severityNew").val("ORANGE");
                                $("#levelNew").val("II");
                                $("#damageNew").val("严重");
                            }else if(flag=="黄色"){
                                $("#severityNew").val("YELLOW");
                                $("#levelNew").val("III");
                                $("#damageNew").val("较重");
                            }else if(flag=="蓝色"){
                                $("#severityNew").val("BLUE");
                                $("#levelNew").val("IV");
                                $("#damageNew").val("一般");
                            }
                        });
                ...
                ,yes: function(index, layero){
                    ...
                    form.on("submit(submitUpdateBtn)", function(data){
                        ...
                        submitServer({
                            ...
                            ,type: 'POST'
                            ,param: data.field
                            ,url: '/client/disasterType/update'
                        });
                    });
                    // 触发表单按钮点击事件
