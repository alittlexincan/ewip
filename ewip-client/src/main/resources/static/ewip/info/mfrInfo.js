...
layui.use(['table','form','laytpl','layer', 'ajaxFileUpload'], function(){
    ...
    table.render({
        ...
        ,cols: [[
            ...
            ,{field: 'mfrCode', title: '山洪沟代码', sort: true}
            ,{field: 'mfrName', title: '山洪沟名称',sort: true}
    ...
    let active = {
        ...
        ,'deleteOption': function (obj) {
            layer.confirm('确定删除该数据？', function(index){
                ...
                submitServer({
                    ...
                    ,param: null
                    ,type: 'DELETE'
                    ,url: '/client/mfrInfo/delete/' + obj.data.id,
                });
            });
        }
        /**
         ...
         */
        ,'downModel': function () {
            window.location.href="/client/mfrInfo/downModel";
        }
        /**
         * 开始导入数据
         */
    ...
    $("#addFile").change(function (e) {
        ...
        if(extension==".xls" || extension==".xlsx"){
            ...
            submitFile({
                async: 'true'
                ,url: '/client/mfrInfo/importExcel'
                ,type: 'POST'
                ,param: null
                ,files: ['addFile']
                ,dataType: 'json'
    ...
    let reloadTable = function (param) {
        table.reload('table', {
            ...
            where: { //设定异步数据接口的额外参数，任意设
                mfrCode: param == undefined ? '' : param.mfrCode
                ,mfrName: param == undefined ? '' : param.mfrName
            }
        });
    };
    /**
