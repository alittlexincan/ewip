...
layui.use(['table','form','laytpl','layer', 'ajaxFileUpload'], function(){
    ...
    table.render({
        ...
        ,cols: [[
            ...
            ,{field: 'tourName', title: '景区名称', sort: true}
    ...
    let active = {
        ...
        ,'deleteOption': function (obj) {
            layer.confirm('确定删除该数据？', function(index){
                ...
                submitServer({
                    ...
                    ,param: null
                    ,type: 'DELETE'
                    ,url: '/client/touInfo/delete/' + obj.data.id,
                });
            });
        }
        /**
         ...
         */
        ,'downModel': function () {
            window.location.href="/client/touInfo/downModel";
        }
        /**
         * 开始导入数据
         */
    ...
    $("#addFile").change(function (e) {
        ...
        if(extension==".xls" || extension==".xlsx"){
            ...
            submitFile({
                async: 'true'
                ,url: '/client/touInfo/importExcel'
                ,type: 'POST'
                ,param: null
                ,files: ['addFile']
                ,dataType: 'json'
    ...
    form.verify({
        tourName: function (value) {
            if(value.length == 0)  return '请填写景区名称';
        }
        ,lon: function (value) {
    ...
    let reloadTable = function (param) {
        table.reload('table', {
            ...
            where: { //设定异步数据接口的额外参数，任意设
                tourName: param == undefined ? '' : param.tourName
            }
        });
    };
    /**
