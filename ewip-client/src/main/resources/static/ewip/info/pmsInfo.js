...
layui.use(['table','form','laytpl','layer', 'ajaxFileUpload'], function(){
    ...
    let typeFormat = function(d){
        if(d.schoolType == 1) return "小学";
        if(d.schoolType == 2) return "初级中学";
        if(d.schoolType == 3) return "高级中学";
        if(d.schoolType == 4) return "职业高中";
        if(d.schoolType == 5) return "中等专业学校";
        if(d.schoolType == 6) return "技工学校";
        if(d.schoolType == 7) return "特殊教育学校";
        if(d.schoolType == 8) return "其他类型";
    };
    ...
    table.render({
        ...
        ,cols: [[
            ...
            ,{field: 'schoolType', title: '学校类型',sort: true,templet:typeFormat}
    ...
    let active = {
        ...
        ,'deleteOption': function (obj) {
            layer.confirm('确定删除该数据？', function(index){
                ...
                submitServer({
                    ...
                    ,param: null
                    ,type: 'DELETE'
                    ,url: '/client/pmsInfo/delete/' + obj.data.id,
                });
            });
        }
        /**
            ...
            layer.open({
                ...
                ,success: function(layero,index){
                    ...
                    laytpl(updatePop.innerHTML).render(param, function(html){
                        ...
                        $("select[name='schoolType']").val(param.schoolType);
                    });
                    form.render();
                }
                ,yes: function(index, layero){
         ...
         */
        ,'downModel': function () {
            window.location.href="/client/pmsInfo/downModel";
        }
        /**
         * 开始导入数据
         */
    ...
    $("#addFile").change(function (e) {
        ...
        if(extension==".xls" || extension==".xlsx"){
            ...
            submitFile({
                async: 'true'
                ,url: '/client/pmsInfo/importExcel'
                ,type: 'POST'
                ,param: null
                ,files: ['addFile']
                ,dataType: 'json'
