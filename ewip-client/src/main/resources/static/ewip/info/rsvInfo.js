...
layui.use(['table','form','laytpl','layer', 'ajaxFileUpload'], function(){
    ...
    table.render({
        ...
        ,cols: [[
            ...
            ,{field: 'reservoirName', title: '水库名称', sort: true}
    ...
    let active = {
        ...
        ,'deleteOption': function (obj) {
            layer.confirm('确定删除该数据？', function(index){
                ...
                submitServer({
                    ...
                    ,param: null
                    ,type: 'DELETE'
                    ,url: '/client/rsvInfo/delete/' + obj.data.id,
                });
            });
        }
        /**
         ...
         */
        ,'downModel': function () {
            window.location.href="/client/rsvInfo/downModel";
        }
        /**
         * 开始导入数据
         */
    ...
    $("#addFile").change(function (e) {
        ...
        if(extension==".xls" || extension==".xlsx"){
            ...
            submitFile({
                async: 'true'
                ,url: '/client/rsvInfo/importExcel'
                ,type: 'POST'
                ,param: null
                ,files: ['addFile']
                ,dataType: 'json'
     ...
     */
    form.verify({
        reservoirName: function (value) {
            if(value.length == 0)  return '请填写水库名称';
        }
        ,lon: function (value) {
    ...
    let reloadTable = function (param) {
        table.reload('table', {
            ...
            where: { //设定异步数据接口的额外参数，任意设
                reservoirName: param == undefined ? '' : param.reservoirName
            }
        });
    };
    /**
