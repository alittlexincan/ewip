...
layui.use(['table','form','laytpl','layer', 'ajaxFileUpload'], function(){
    ...
    table.render({
        ...
        ,cols: [[
            ...
            ,{field: 'flaExpPlaceName', title: '场所名称', sort: true}
    ...
    let active = {
        ...
        ,'deleteOption': function (obj) {
            layer.confirm('确定删除该数据？', function(index){
                ...
                submitServer({
                    ...
                    ,param: null
                    ,type: 'DELETE'
                    ,url: '/client/feInfo/delete/' + obj.data.id,
                });
            });
        }
        /**
         ...
         */
        ,'downModel': function () {
            window.location.href="/client/feInfo/downModel";
        }
        /**
         * 开始导入数据
         */
     ...
     */
    form.verify({
        flaExpPlaceName: function (value) {
            if(value.length == 0)  return '请填写场所名称';
        }
        ,lon: function (value) {
    ...
    let reloadTable = function (param) {
        table.reload('table', {
            ...
            where: { //设定异步数据接口的额外参数，任意设
                flaExpPlaceName: param == undefined ? '' : param.flaExpPlaceName
            }
        });
    };
    /**
