...
layui.use(['table','form','laytpl','layer', 'ajaxFileUpload'], function(){
    ...
    table.render({
        ...
        ,cols: [[
            ...
            ,{field: 'geoCode', title: '地灾点编号', sort: true}
            ,{field: 'geoName', title: '地灾点名称',sort: true}
            ,{field: 'geoType', title: '灾种类型',sort: true}
            ,{field: 'lon', title: '经度', sort: true}
            ,{field: 'lat', title: '纬度',sort: true}
            ,{field: 'respPerson', title: '防灾责任人', sort: true}
            ,{field: 'respMobilePhone', title: '防灾责任人电话', sort: true}
    ...
    let active = {
        ...
        ,'deleteOption': function (obj) {
            layer.confirm('确定删除该数据？', function(index){
                ...
                submitServer({
                    ...
                    ,param: null
                    ,type: 'DELETE'
                    ,url: '/client/geoInfo/delete/' + obj.data.id,
                });
            });
        }
        /**
            ...
            layer.open({
                ...
                ,success: function(layero,index){
                    ...
                    laytpl(updatePop.innerHTML).render(param, function(html){
                        ...
                        $("select[name='dmFactor']").val(param.dmFactor);
                        $("select[name='rainfallFactor']").val(param.rainfallFactor);
                        $("select[name='waterLineFactor']").val(param.waterLineFactor);
                        $("select[name='soilFactor']").val(param.soilFactor);
                    });
                    form.render();
                }
                ,yes: function(index, layero){
         ...
         */
        ,'downModel': function () {
            window.location.href="/client/geoInfo/downModel";
        }
        /**
         * 开始导入数据
         */
    ...
    $("#addFile").change(function (e) {
        ...
        if(extension==".xls" || extension==".xlsx"){
            ...
            submitFile({
                async: 'true'
                ,url: '/client/geoInfo/importExcel'
                ,type: 'POST'
                ,param: null
                ,files: ['addFile']
                ,dataType: 'json'
    ...
    let reloadTable = function (param) {
        table.reload('table', {
            ...
            where: { //设定异步数据接口的额外参数，任意设
                geoCode: param == undefined ? '' : param.geoCode
                ,geoName: param == undefined ? '' : param.geoName
            }
        });
    };
    /**
