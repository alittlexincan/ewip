...
public class RsvExcelUtil {
    ...
    public static List<Map<String,Object>> getData(Sheet sheet){
        ...
        if (!sheet.getRow(0).getCell(0).toString().equals("水库名称")
                && !sheet.getRow(0).getCell(1).toString().equals("经度")){
            return null;
        }
        for(Row row : sheet){
            // 跳过第一行的目录
            ...
            if(!getValue(row.getCell(0)).toString().equals("")
                ...
                map.put("reservoirName",getValue(row.getCell(0)).toString());
                map.put("lon",getValue(row.getCell(1)));
                map.put("lat",getValue(row.getCell(2)));
                map.put("contact",getValue(row.getCell(3)).toString());
                map.put("mobilePhone",getValue(row.getCell(4)).toString());
                list.add(map);
            }
        }
