...
public class HosExcelUtil {
    ...
    public static List<Map<String,Object>> getData(Sheet sheet){
        ...
        if (!sheet.getRow(0).getCell(0).toString().equals("医院名称")
                && !sheet.getRow(0).getCell(1).toString().equals("医院等级")){
            return null;
        }
        for(Row row : sheet){
            // 跳过第一行的目录
            ...
            if(!getValue(row.getCell(0)).toString().equals("")
                ...
                map.put("hospitalName",getValue(row.getCell(0)).toString());
                map.put("hospitalLevel",getValue(row.getCell(1)).toString());
