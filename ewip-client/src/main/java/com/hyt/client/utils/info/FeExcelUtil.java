...
public class FeExcelUtil {
    ...
    public static List<Map<String,Object>> getData(Sheet sheet){
        ...
        if (!sheet.getRow(0).getCell(0).toString().equals("场所名称")
                && !sheet.getRow(0).getCell(1).toString().equals("场所管理单位")){
            return null;
        }
        for(Row row : sheet){
            // 跳过第一行的目录
            ...
            if(!getValue(row.getCell(0)).toString().equals("")
                ...
                map.put("flaExpPlaceName",getValue(row.getCell(0)).toString());
                map.put("unit",getValue(row.getCell(1)).toString());
