...
layui.use(["table","form","laytpl","layer","selectTree","zTree"], function(){
    ...
    let statusFormat = function(d){
        if(d.status == 0) return "<span class='layui-btn layui-btn-warm layui-btn-xs ocpp-cursor-default'>禁用</span>";
        if(d.status == 1) return "<span class='layui-btn layui-btn-xs ocpp-cursor-default'>启用</span>";
    };
