...
public class RiskWaterloggingController {
    ...
    @ApiOperation(value = "查询内涝隐患点信息列表", httpMethod = "GET", notes = "查询所有内涝隐患点信息")
    @GetMapping("/list")
    public ResultObject<Object> selectList(@ApiParam(hidden = true) @RequestParam Map<String,Object> map) {
        List<RiskWaterlogging> list = this.riskWaterloggingService.selectList(map);
        if(list.size()>0){
            return ResultResponse.make(200,"查询内涝隐患点成功",list);
        }
        return ResultResponse.make(500,"查询内涝隐患点失败",null);
    }
}
