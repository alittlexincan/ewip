...
public class BiInfoController {
    ...
    public ResultObject<Object> insert(@RequestParam Map<String,Object> map){
        ...
        BiInfo biInfo = JSON.parseObject(json.toJSONString(), new TypeReference<BiInfo>() {});
        int num = this.biInfoService.insert(biInfo);
        if(num>0){
            return ResultResponse.make(200,"添加成功",biInfo);
        }
    ...
    public ResultObject<Object> update(@RequestParam Map<String,Object> map){
        JSONObject json = new JSONObject(map);
        BiInfo biInfo = JSON.parseObject(json.toJSONString(), new TypeReference<BiInfo>() {});
        int num = this.biInfoService.update(biInfo);
        if(num>0){
    ...
    public ResultObject<Object> deleteById(@PathVariable(value = "id") String id) {
        Integer num = this.biInfoService.deleteById(id);
        if(num>0){
    ...
    public ResultObject<Object> deleteBatch(@RequestParam(value = "id") String id) {
        Integer num = this.biInfoService.deleteByIds(id);
        if(num>0){
            return  ResultResponse.make(200,"删除成功");
        }
        return ResultResponse.make(500,"删除失败");
    ...
    public ResultObject<Object> selectAll( @RequestParam Map<String,Object> map) {
        PageInfo<BiInfo> pageInfo = this.biInfoService.selectAll(map);
    ...
    public ResultObject<Object> selectList(@RequestParam Map<String,Object> map){
        List<BiInfo> list = this.biInfoService.selectList(map);
        if(list.size()>0){
    ...
    public JSONObject importData(@RequestParam Map<String,Object> map, @RequestBody List<Map<String,Object>> list) {
        JSONObject json=this.biInfoService.importData(map,list);
        return json;
    }
